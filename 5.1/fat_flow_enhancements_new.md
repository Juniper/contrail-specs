...
in the use case given below. An intermediate solution is required which can give reasonable entropy of flows as well as conserve 
...
Src/Dst IP address is to be completely ignored. The convention of Src and Dst will be same as existing convention implemented 
...
3. Propagate new fat flow config information at VN level to VMI level (TBD if any changes are actually required here).
