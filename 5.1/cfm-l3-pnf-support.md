...
- When the user creates the service template for PNF, user must have a knob to select whether Left and Right service VRFs are Shared (across all network policies using an 
   instance created) or not shared
...
- Service Template object should specify whether service instance created from this template will be SHARED (one and only one Left and Right VFR instance will be cerated for 
  all inter VN network policies) or not SHARED (for each inter VN network policy one Left and one Right VRF instance will be created)
- Service Instance object should specify the Logical Routers that are to be attached to the Left and Right side of the PNF
- Service Instance object should specify the list of on-boarded PNFs - PNFs that compose that service instance
...
- User also selects from a list of previously on-boarded PNFs - the PNFs that compose that service instance
...
- User also selects whether service instance created from this template will be SHARED (one and only one Left and Right VFR instance will be cerated for 
  all inter VN network policies) or not SHARED (for each inter VN network policy one Left and one Right VRF instance will be created)
...
- User then selects the logical routers that are to be attached to the Left and Right side of the PNF device
- User then selects from a list of previously onboard PNFs - PNFs that compose that service instance
