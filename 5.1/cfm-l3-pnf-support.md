...
- User also needs to select the PNF device from a pick list of physical routers o-nboarded previously (See 4.1). This pick list should only show those devices assigned with "PNF" physical role and "firewall" overlay role.
...
- User then selects the logical routers that are to be attached to the Left and Right side of the PNF device along with Left and Right side of QFX device. The API server will validate to make sure the selected logical routers are from the same physical spine device which connects to a PNF device defined in the service templates. If validation succeeds, the reference from left attachment point to the spine device's left/right service interfaces are created.
