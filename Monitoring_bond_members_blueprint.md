...
# 7. Limitations
1. When Master interface goes down in DPDK(with bond/without bond), WEbUI(contrail command UI) interface status information would be incorrect. When master interface goes down, control-data interface would not be available to provide the updated status to control module(controller).
