...
## 10.4 Configure new device serial number and hostname
...
2. If desired, enter the hostname of the new device on the device information page in the UI.
If the hostname is not set here, it can optionally be set in the next step.

## 10.5 Put device into RMA_ACTIVATE state
1. UI calls rma_activate device workflow via execute-job API with device UUID and optional serial_number and hostname input
2. Device Manager starts rma_activate device job and rma_activate Ansible playbook invoked:
   ...
   - Update hostname if provided with input. This involves replacing the existing physical_router object in the database since the fq_name is now different.
