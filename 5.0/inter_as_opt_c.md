...
### 4.1.2 Agent
Agent exports vhost rutes with label 3 (implicit NULL( so that received
packets processing is similar to current tunnelled packtes as ASBR sends
...
as component nexthops for ECMP cases.Labelled tubbel nexthop is resolved by
...
Agent subscribes to global system configuration so that it will advertise
...
inet.3 routes for it's route resolution, then inet.3 route update trigger
L3VPN route reevaluation and which inturn trigger reevaluation for dependent
flows.
